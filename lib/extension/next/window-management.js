...
export class WindowManagement extends GObject.Object {
  ...
  /**
   * This is where Forge adds metadata and signals to be able to manage it.
   * This is normally called during `window-created` event.
   *
   * @param {Meta.Window} metaWindow
   */
  ...
  /**
   * Track existing windows. This is useful when the extension has been disabled
   * and then re-enabled, installed for the first time or coming out of lock screen.
   *
   */
  ...
  /**
   * Check whether a particular window is allowed to be handled by Forge.
   * Temporary windows such as tooltips on hover are a waste to track because
  ...
  /**
   * Remove a Meta.Window by its Meta.WindowActor or Clutter.Actor instance.
   *
  ...
  #forgedWindow(metaWindow) {
    ...
    return {
      ...
      // Note, these two should not be part of the serialization data
