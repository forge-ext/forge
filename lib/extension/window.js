...
export class WindowManager extends GObject.Object {
  ...
  addFloatOverride(metaWindow, byClass, nonPersistent) {
    ...
    for (let override in overrides) {
      ...
      if (override.wmClass === wmClass &&
        override.mode === "float" &&
        (
          (byClass && !override.wmTitle) ||
          (!byClass && override.wmTitle === wmTitle)
        )
      ) return;
    }
    if (nonPersistent) {
  ...
  removeFloatOverride(metaWindow, byClass, nonPersistent) {
    let overrides = this.windowProps.overrides;
    let wmTitle = metaWindow.get_title();
    let wmClass = metaWindow.get_wm_class();
    let wmId = metaWindow.get_id();
  ...
  toggleFloatingMode(action, metaWindow) {
    ...
    let nonPersistent = action.name === "FloatNonPersistentToggle";
    let byClass = action.name === "FloatClassToggle";
    let floatingExempt = this.isFloatingExempt(metaWindow);

    if (floatingExempt) {
      this.removeFloatOverride(metaWindow, byClass, nonPersistent);
      ...
      this.addFloatOverride(metaWindow, byClass, nonPersistent);
      nodeWindow.mode = WINDOW_MODES.FLOAT;
    }
  }
