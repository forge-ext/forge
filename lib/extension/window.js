  ...
  showWindowBorders() {
    ...
    let maximized = () => {
      try {
        ...
        // pre-49 fallback
        return metaWindow.is_maximized() === 3 || metaWindow.is_fullscreen() || gap === 0;
      }
    };
    ...
    if (
      ...
      (() => {
        try {
          ...
          // pre-49 fallback
          return metaWindow.is_maximized() === 1 || metaWindow.is_maximized() === 2;
        }
      })()
    ) {
      inset = 0;
      ...
      if (
        (() => {
          try {
            ...
            // pre-49 fallback
            return focusMetaWindow.is_maximized() === 0;
          }
        })()
      ) {
        this.renderTree(from);
        ...
        monitor.getNodeByType(NODE_TYPES.WINDOW).filter((w) => {
          return (() => {
            try {
              ...
              // pre-49 fallback
              return (
                w.nodeValue.is_maximized() === Meta.MaximizeFlags.BOTH ||
