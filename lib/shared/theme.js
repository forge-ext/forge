...
// TODO: subclass this for the extension and prefs versions, rather than passing
// in a flag
export class ThemeManager extends GObject.Object {
  ...
  constructor({ configMgr, settings, dir, metadata, stylesheet }, options = { prefsMode: false }) {
    super();
    this.configMgr = configMgr;
    this.settings = settings;
    this.metadata = metadata;
    this.stylesheet = stylesheet;
    if (options.prefsMode) this.extensionPath = `${dir.get_path()}`;
  ...
  _importCss() {
    ...
    if (success) {
      const cssContents = TextDecoder.decode(contents);
